...
Public Class JAZZ_CASH_FRM
    ...
    Sub UPDATE_TRANS_CMD()
        'Using DR USING DATA CONNECTION
        Using con As New MySqlConnection("server=localhost; username=root; password=Masoom1; database=easypaisa_db;")
            Using cmd As New MySqlCommand("INSERT INTO `TRANS_ID` VALUES (NULL,'" & CUSTOMER_NAME_TXT.Text & "','" & AMOUNT_TXT.Text & "','" & TNO_TXT.Text & "','" & Date.Today.ToString("dd-MM-yyyy") & "','JAZZ CASH');", con)
                Using da As New MySqlDataAdapter
                    con.Open()
                    da.SelectCommand = cmd
                    cmd.ExecuteNonQuery()
